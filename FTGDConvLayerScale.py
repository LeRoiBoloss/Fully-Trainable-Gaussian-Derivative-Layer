...
class MaxGroupPool(tensorflow.keras.layers.Layer):
    def __init__(self, num_scales, **kwargs):
    ...
    def call(self, inputs):
        ...
        output = tensorflow.reshape(inputs, [-1, self.inputShape[1], self.inputShape[2], self.num_scales, self.inputShape[3] // self.num_scales])
...
class FTGDConvLayerScaleLifting(tensorflow.keras.layers.Layer):
    """
    ...
    def __init__(self, filters, kernel_size,  num_basis, order, scales=[1], separated = False, trainability = [True, True, True], padding = 'same', sigma_init = 1, mu_init = 0, theta_init = 0, strides = (1,1), random_init = True, use_bias = False, **kwargs):

        super(FTGDConvLayerScaleLifting, self).__init__()
        self.scales = scales
    ...
    def call(self, inputs):
        ...
        if self.deployed:
            ...
            if self.separated:
                scaled_outputs = [computeOutput([ScaledGaussFilters, self.clWeights[1]], inputs, self.num_basis, self.separated, self.padding_mode, self.stride) for ScaledGaussFilters in self.GaussFilters]

            else:
                scaled_outputs = [computeOutput(ScaledGaussFilters, inputs, self.num_basis, self.separated, self.padding_mode, self.stride) for ScaledGaussFilters in self.GaussFilters]

            if self.use_bias:
                scaled_outputs = [nn.bias_add(outputs, self.bias, data_format='NHWC') for outputs in scaled_outputs]

            scaled_outputs = tensorflow.concat(scaled_outputs, -1)

            return scaled_outputs

        else:
            size_scale_normalized = [(int(self.filter_size[0]*scale), int(self.filter_size[0]*scale)/self.filter_size[0]) for scale in self.scales]
            
            GaussFilters = [getGaussianFilters(getBases(size, self.num_basis, self.order, self.sigmas*scale, self.centroids*scale, self.thetas), self.clWeights, self.num_basis, self.inputChannels, self.num_filters, self.separated) for size, scale in size_scale_normalized]
            ...
            if self.separated:
                ...
                scaled_outputs = [computeOutput(ScaledGaussFilters, inputs, self.num_basis, self.separated, self.padding_mode, self.stride) for ScaledGaussFilters in GaussFilters]
    ...
    def get_config(self):
        config = super(FTGDConvLayerScaleLifting, self).get_config()
        config.update({
            "scales":self.scales,
